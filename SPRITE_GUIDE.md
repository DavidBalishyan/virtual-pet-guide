...
If you want the nitty-gritty on why images are stored the way they are (RGB565, PROGMEM,
transparent colour keys), check the **"How Sprite Images Are Stored and Drawn"** section
in `USEFUL_NOTES.md`.
...
```
1. Draw your sprite in Piskel (piskelapp.com, 80x80 canvas)
 |
...
The canvas is now 80x80 pixels - this project's sprite size. Each small square you paint is
...
- **3 - 4 colours per sprite is usually enough.** At 80x80 on this small screen, subtle colour
...
These rules matter now, while you're drawing. Break them and you'll get visual bugs that
are a pain to track down once the rendering code is in place.

### Rule 1 - All sprites must be the same size

Every sprite must be the **same size as the others**. This project uses **80x80 pixels** - 
all four mood sprites are 80x80. If neutral is 80x80 and happy is 48x48, the rendering code
...
80x80 sprite with a single `pushImage()` at 80x80). The converter handles any size - it reads
...
project's 80x80 size:

```
4 moods x 1 frame x 80 pixels wide x 80 pixels tall x 2 bytes per pixel
= 51,200 bytes ~ 50 KB
```

The ESP32 has roughly 2.5 MB of flash available after the firmware, so ~50 KB for the four
single-frame mood sprites is comfortable. Animation multiplies the cost - it is `frames x
...
The pet sprite is **80x80 pixels, drawn at native size - the renderer does not scale it**.
...
| +-----------+ |
| | | |
| | 80 x 80px | | 80x80, drawn at native size (1x)
...
The interact screen shows the pet at the top - at the **same 80x80 native size and the same
...
- The pet is 80x80 at native size, face centre y = 110 - identical to MAIN, so it never jumps
...
|----------|--------------------|-------------|-----------------|--------------------------|
| MAIN | 80 x 80 px | native (1x) | y = 110 | title above y=24, mood at y=155 |
| STATS | no sprite | - | - | data only: bars to y=134, mood word at y=180 |
| INTERACT | 80 x 80 px | native (1x) | y = 110 | title above y=24, mood at y=155 |

**The key takeaway:** the pet sprite is **80x80, drawn at native size** on the MAIN and
...
```
total bytes = width x height x frames_per_sprite x number_of_sprites x 2
```

This project's chosen size is **80x80**, and it ships four single-frame mood sprites - the
80x80 row below, in its "x1 frame" column, is the one that matches the firmware. The wider
table is kept as a general reference: it shows flash cost at each base size from 32x32 up to
135x135 (the largest square that fits the 135 px screen width), and how cost grows as you add
...
| Base size | Per frame | 7 states x 1 frame | 7 states x 4 frames | 7 states x 8 frames | Budget |
|-----------|-----------|--------------------|---------------------|---------------------|--------|
| 32 x 32 | 2 KB | 14 KB | 56 KB | 112 KB | OK |
| 48 x 48 | 4.5 KB | 31 KB | 126 KB | 252 KB | OK |
| 64 x 64 | 8 KB | 56 KB | 224 KB | 448 KB | OK |
| 80 x 80 | 12.5 KB | 88 KB | 350 KB | 700 KB | (tight) |
| 96 x 96 | 18 KB | 126 KB | 504 KB | 1,008 KB | X |
| 112 x 112 | 24.5 KB | 172 KB | 686 KB | 1,372 KB | X |
| 128 x 128 | 32 KB | 224 KB | 896 KB | 1,792 KB | X |
| 135 x 135 | 35.6 KB | 249 KB | 997 KB | 1,993 KB | X |

**Budget key:**
- OK safe - 7 states x 8 frames fits well within the ~812 KB of free flash
- (tight) tight - 7 states x 8 frames uses ~700 KB; reduce frame count or state count to stay safe
- X over budget for 8 frames - keep to 4 frames per state max (96x96 at 4 frames = 504 KB OK)
...
sprite from 32x32 to 64x64 quadruples the pixel count (1,024 -> 4,096) and quadruples
the flash cost. A jump from 64x64 to 128x128 costs 16x as much as the original 32x32.
...
|-----------|------------------------------------|-------|
| 64 x 64 | fits with 26 px spare | no sprite - no limit |
| 80 x 80 | fits with 10 px spare (the chosen size) | no sprite - no limit |
| 96 x 96 | clips (6 px over) | no sprite - no limit |
| 112 x 112 | clips (22 px over) | no sprite - no limit |
| 128 x 128 | clips (38 px over) | no sprite - no limit |
...
oversized one. The shipped **80x80** size fits the band with 10 px to spare, which is why it
was chosen.
...
Three art rules make an animation read well on a small 135x240 screen:
...
**Never do this.** `delay(200)` freezes the *whole* program for 200 ms - buttons stop working,
sound stops, the stat timers stop. On a microcontroller running one big `loop()`, putting `delay()` inside
...
| Frame | Gap (`target - smoothed`) | Step (gap x 0.2) | New `smoothedX` |
|------:|--------------------------:|-----------------:|----------------:|
| 1 | 100 | 20 | 20 |
| 2 | 80 | 16 | 36 |
| 3 | 64 | 12.8 | 48.8 |
...
On flash cost: four 80x80 sprites at 12.8 KB each is ~51 KB, and even an animated set (eight frames)
