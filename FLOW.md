...
Runs over and over after `setup()` finishes. `loop()` itself is short - it hands off to three named helpers, so each frame reads like a clear outline:
...
2. **Run the state machine** - `myPet.updateState(speaker)` advances the pet's state, sets `STATE_DEAD` when fullness, energy, or happiness reaches 0, **and** plays the matching low-fullness / sickness / death sound when a stat crosses its alert threshold. The pet handles its own audio here - `main.cpp` never reads alert flags or calls the speaker directly.
...
When the `DEBUG` flag is defined, the loop also calls `printPetStateToSerial()` on its own throttled timer (so it doesn't flood the monitor) to dump the pet's stats for debugging. It compiles out entirely when `DEBUG` is off.
